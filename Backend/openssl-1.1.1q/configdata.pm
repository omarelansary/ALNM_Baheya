...
our %config = (
  ...
  openssldir => "/etc/ssl",
  options => "--prefix=/usr --openssldir=/etc/ssl --libdir=lib enable-zlib-dynamic no-afalgeng no-asan no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers",
  perl_archname => "darwin-thread-multi-2level",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.30.3",
  perlargv => [ "darwin64-arm64-cc", "--prefix=/usr", "--openssldir=/etc/ssl", "--libdir=lib", "no-shared", "zlib-dynamic" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
